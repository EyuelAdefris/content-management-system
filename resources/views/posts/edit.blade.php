...
<div class="mb-6 flex items-center justify-between">
    <h2 class="text-2xl font-bold text-slate-800">Edit Post</h2>
    ...
    <form method="POST" action="{{ route('posts.update', $post) }}" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            <label for="excerpt" class="block text-sm font-medium text-slate-700 mb-1.5">
                Excerpt <span class="text-slate-400 font-normal">(optional)</span>
            </label>
            <textarea id="excerpt"
                      name="excerpt"
                      rows="3"
                      placeholder="Short description or summary..."
                      class="w-full px-4 py-2.5 border {{ $errors->has('excerpt') ? 'border-red-400 bg-red-50' : 'border-slate-300' }} rounded-lg text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 resize-y">{{ old('excerpt', $post->excerpt) }}</textarea>
            @error('excerpt')
                <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea id="content"
                      ...
                      class="w-full px-4 py-2.5 border {{ $errors->has('content') ? 'border-red-400 bg-red-50' : 'border-slate-300' }} rounded-lg text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 resize-y">{{ old('content', $post->content) }}</textarea>
            @error('content')
                <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        {{-- Featured Image --}}
        <div class="mb-6">
            <label for="featured_image" class="block text-sm font-medium text-slate-700 mb-1.5">
                Featured Image <span class="text-slate-400 font-normal">(optional, jpg/png/webp, max 2MB)</span>
            </label>
            ...
            @if($post->featured_image)
                <div class="mb-3 flex items-center space-x-4">
                    ...
                    <div>
                        <p class="text-xs text-slate-500 font-medium">Current image</p>
                        <p class="text-xs text-slate-400 mt-0.5">Upload a new file below to replace it.</p>
                    </div>
                </div>
            @endif
            ...
            <input type="file"
                   ...
                   name="featured_image"
                   accept="image/*"
                   class="w-full px-4 py-2.5 border {{ $errors->has('featured_image') ? 'border-red-400 bg-red-50' : 'border-slate-300' }} rounded-lg text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:text-xs file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
