    ...
    <form method="POST" action="{{ route('posts.update', $post) }}" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            <textarea id="excerpt"
                      ...
                      class="w-full px-4 py-2.5 border {{ $errors->has('excerpt') ? 'border-red-400 bg-red-50' : 'border-slate-300' }} rounded-lg text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-150 resize-y">{{ old('excerpt', $post->excerpt) }}</textarea>
            @error('excerpt')
                <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea id="content"
                      ...
                      class="w-full px-4 py-2.5 border {{ $errors->has('content') ? 'border-red-400 bg-red-50' : 'border-slate-300' }} rounded-lg text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent transition duration-150 resize-y">{{ old('content', $post->content) }}</textarea>
            @error('content')
                <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p>
            @enderror
        </div>
