...
<div class="mb-6 bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-4">
    <form action="{{ route('posts.index') }}" method="GET" class="flex flex-col sm:flex-row sm:items-center gap-4">
        <div class="flex-1">
        ...
        <div class="flex space-x-2">
            ...
            @if(request()->hasAny(['search', 'status']))
                <a href="{{ route('posts.index') }}" class="px-4 py-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 text-sm font-medium rounded-lg transition duration-150">
                    Clear
                </a>
            @endif
        </div>
            ...
            <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50">
                ...
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-700/50 transition duration-100">
                    <td class="px-6 py-4">
                        ...
                        @else
                            <div class="w-12 h-12 rounded-lg bg-slate-100 dark:bg-slate-700 border border-slate-200 dark:border-slate-600 flex items-center justify-center">
                                <svg class="h-5 w-5 text-slate-300 dark:text-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <td class="px-6 py-4">
                        <p class="text-sm font-medium text-slate-800 dark:text-slate-200">{{ $post->title }}</p>
                        @if($post->excerpt)
                            <p class="text-xs text-slate-400 dark:text-slate-500 mt-0.5 truncate max-w-xs">{{ $post->excerpt }}</p>
                        @endif
                    </td>
                    <td class="px-6 py-4">
                    ...
                    <td class="px-6 py-4 text-sm text-slate-600 dark:text-slate-400">{{ $post->author->name ?? 'Unknown' }}</td>
                    <td class="px-6 py-4 text-sm text-slate-500 dark:text-slate-400">{{ $post->created_at->format('M d, Y') }}</td>
