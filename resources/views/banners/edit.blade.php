...
<div class="mb-6 flex items-center justify-between">
    <h2 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Banner</h2>
    ...
    <form method="POST" action="{{ route('banners.update', $banner) }}" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            <label for="image" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">
                Upload New Image <span class="text-slate-400 dark:text-slate-500 font-normal">(optional, jpg/png/webp, max 2MB)</span>
            </label>
            <input type="file"
                   id="image"
                   name="image"
                   accept="image/*"
                   class="w-full px-4 py-2.5 border {{ $errors->has('image') ? 'border-red-400 bg-red-50 dark:border-red-500/50 dark:bg-red-950/20' : 'border-slate-300 dark:border-slate-700' }} rounded-lg text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 bg-white dark:bg-slate-800 file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:text-xs file:font-medium file:bg-blue-50 dark:file:bg-blue-950/50 file:text-blue-700 dark:file:text-blue-400 hover:file:bg-blue-100 dark:hover:file:bg-blue-900/50">
            @error('image')
                <p class="mt-1.5 text-xs text-red-600 dark:text-red-400 font-medium">{{ $message }}</p>
            @enderror
        </div>
