    ...
    <form method="POST" action="{{ route('banners.update', $banner) }}" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            <input type="file"
                   ...
                   name="image"
                   accept="image/*"
                   class="w-full px-4 py-2.5 border {{ $errors->has('image') ? 'border-red-400 bg-red-50' : 'border-slate-300' }} rounded-lg text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition duration-150 file:mr-3 file:py-1 file:px-3 file:rounded file:border-0 file:text-xs file:font-medium file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
            @error('image')
                <p class="mt-1.5 text-xs text-red-600 font-medium">{{ $message }}</p>
            @enderror
        </div>
